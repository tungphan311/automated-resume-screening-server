        ...
        if not account:
            try:
                ...
                if account_inserted:
                    # send email here
                    try:
                        confirm_url = url_for('api.Candidate_candidate_verify',token=account_inserted.access_token, _external=True)
    ...
    def get(self, token):
        ...
        try:
            ...
            if jwt_data and ('identity' in jwt_data) and datetime.datetime.now().timestamp() < jwt_data['exp']:
                ...
                account = get_a_account_candidate_by_email(jwt_data['identity'])
